window.addEventListener("resize", setupCanvas);
  ...
  // Ajusta o estilo do canvas para ficar no fundo
  canvas.style.position = "fixed"; // Fica fixo mesmo ao rolar a página
  canvas.style.left = "0px";
  canvas.style.top = "0px";
  canvas.style.width = "100vw";
  canvas.style.height = "100vh";
  canvas.style.zIndex = "-999"; // Coloca o canvas atrás de todo o conteúdo
  ...
  function setupLizard(size, legs, tail) {
    ...
    for (var i = 0; i < 32; i++) {
      ...
      for (var ii = -1; ii <= 1; ii += 2) {
        var node = new Segment(spinal, s * 3, ii, 0.1, 2);
        for (var iii = 0; iii < 3; iii++) {
          node = new Segment(node, s * 3 * (32 - i) / 32, -ii * 0.1, 0.1, 2);
        }
      }
    }
    setInterval(function() {
